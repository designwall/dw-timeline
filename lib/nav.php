...
class dw_timeline_Nav_Walker extends Walker_Nav_Menu {
  ...
  function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
    ...
    if ( ! $item->is_dropdown && $depth === 0 ) {
      $item_html = str_replace('<a', '<a data-hover="'.$item->title.'" ', $item_html);
    } elseif ($item->is_dropdown && ($depth === 0)) {
      $item_html = str_replace('<a', '<a class="dropdown-toggle" data-toggle="dropdown" data-hover="'.$item->title.'" data-target="#"', $item_html);
...
function dw_timeline_nav_menu_args($args = '') {
  ...
  if (current_theme_supports('bootstrap-top-navbar') && !$args['depth']) {
    $dw_timeline_nav_menu_args['depth'] = 2;
  }
